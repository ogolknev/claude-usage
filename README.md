...
Лёгкий нативный индикатор в строке меню: показывает реальные лимиты Claude
(сессионный 5ч и недельный) и время сброса — то же, что `/usage` в Claude Code,
плюс локальный расход токенов из логов как фолбэк.

Панель: кольцо-прогрессбар по сессионному (5ч) лимиту с плавным градиентом цвета
(зелёный→жёлтый→красный) и процентом в стиле индикатора батареи, напр. `◔ 13 %`.
...
## Стоимость (API-эквивалент)

В меню — стоимость расхода по **официальным тарифам Anthropic**: input/output на
модель, а кэш выводится по формуле от input (write-5мин ×1.25, write-1час ×2,
read ×0.1). Запись кэша у Claude Code 1-часовая, это учитывается. На подписке ты
это не платишь — цифра показывает «сколько стоило бы по API».

Цены держатся актуальными автоматически: раз в сутки `pricing.rs` подтягивает
input/output из [LiteLLM](https://github.com/BerriAI/litellm) (зеркалит офф-цены,
тот же источник, что у `ccusage`) в `~/Library/Caches/com.local.claude-usage/prices.json`.
Базовые цены вшиты как фолбэк, если обновление не прошло.

## Структура

```
src/
  main.rs      событийный цикл tao + tray-icon, опрос с бэкоффом, меню, --probe
  limits.rs    /api/oauth/usage + парсинг (единственная точка риска)
  keychain.rs  чтение Claude Code-credentials через /usr/bin/security
  local.rs     расход из JSONL-логов
  pricing.rs   офф-тарифы Anthropic + авто-обновление из LiteLLM
  icon.rs      рендер кольца-прогрессбара (RGBA, HSV-градиент)
  cache.rs     кэш последних удачных лимитов на диске
  tray.rs      форматирование строки и пунктов меню
  model.rs     общие структуры
bundle/        Info.plist (LSUIElement), build-app.sh, build-dmg.sh, autostart.sh
```
